...
class NeuralNetwork(object):
    ...
    def predict(self, X, w_matrix=None, model="lgd", batch=100):
        ...
        for n in range(self.iterations):
            ...
            for index in range(self.X.shape[0]):
                ...
                if max(hypo) <= 10000:
                    ...
                    exponential_value = self.bigfloat_exp(hypo)

                prob = exponential_value[int(y)] / np.sum(exponential_value)

                gradient_matrix[int(y),:] = gradient_matrix[int(y),:] + x*(1-prob)
            ...
            if n == self.iterations - 1:
                ...
                self.min_weight_matrix = self.weight_multiclass[np.argmin(self.gradient_norm)]
                self.multi_weight_matrix = weight_multiclass
                return self.min_weight_matrix #weight_multiclass
