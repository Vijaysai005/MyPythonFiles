...
class GoogleMapPlotter(object):
    ...
    def __init__(self, center_lat, center_lng, zoom, apikey='', folder="/"):
    ...
    def marker(self, lat, lng, color='#FF0000', c=None, title="no implementation", string="No Data"):
        if c:
            color = c
        color = self.color_dict.get(color, color)
        color = self.html_color_codes.get(color, color)
        self.points.append((lat, lng, color[1:], title, string))

    def scatter(self, lats, lngs, color=None, size=None, marker=True, c=None, s=None, string="No Data", **kwargs):
        color = color or c
        ...
        for lat, lng in zip(lats, lngs):
            if marker:
                if string == "No Data":
                    self.marker(lat, lng, settings['color'], string=string)
                else:
                    self.marker(lat, lng, settings['color'], string=string[i])
                    i+=1
            else:
    ...
    def plot(self, lats, lngs, color=None, c=None, string="\"No Data\"", **kwargs):
        color = color or c
        kwargs.setdefault("color", color)
        settings = self._process_kwargs(kwargs)
        path = zip(lats, lngs)
        ...
        if string == "\"No Data\"":
            self.marker(lats[0], lngs[0], "#32CD32", string=string)
        else:
            self.marker(lats[0], lngs[0], "#32CD32", string=string[0])
        if string == "\"No Data\"":
            self.marker(lats[-1], lngs[-1], "#8B0000", string=string)
        else:
            self.marker(lats[-1], lngs[-1], "#8B0000", string=string[0])
    ...
    def draw(self, htmlfile):
        ...
        f.write('<title>GeoUtils - Maps </title>\n')
        if self.apikey:
    ...
    def write_point(self, f, lat, lon, color, title, string="No Data"):
